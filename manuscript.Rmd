...
female <- as.tibble(des.2$Female)
male <- as.tibble(des.2$Male)

```

A cross-sectional survey was used to collect data from a large sample of students of grade 8-10 (n = `r nrow(efa.data)`) from ---- schools following convenience sampling method. T. For estimating the sample size for the CFA we followed the N:q rule [@RN1268; @RN1269; @RN1270; @RN1271] where 10 participants per parameters is required to earn trustworthiness of the result. Our sample size exceeds the requirement. Among `r nrow(data)` participants, `r female[1,2]` were female aged between `r female[4,8]` to `r female[4,9]` years (`r round(female[4,3],2)`±`r round(female[4,4],2)`). `r male[1,2]`  were male with an age range between `r male[4,8]` to `r male[4,9]` years (`r round(male[4,3],2)`±`r round(male[4,4],2)`). Table \@ref(tab:demographics) summarizes the demographics.
